...
from registro_paises import Paises
...
def validar_anfitrion(vec_paises):
    ...
    """
    anfi = input("Ingrese el país anfitrión: ")
...
def grabar_archivo_confederacion(vec_confederacion, archivo):
    ...
    for i in range(len(vec_confederacion)):
        ...
        c += 1
    print("\nEl archivo", archivo, "está cargado","y se grabaron",c,
          "registros")
    m.close()
...
def buscar_archivo(archivo, vec_conf):
    ...
    if not os.path.exists(archivo):
        nuevo = input("El archivo no está ¿Desea escribir uno nuevo?("
                      "S/N): ")
        if nuevo == "S":
            print("El archivo",archivo,"está. Este es su contenido\n")
...
# Opcion 6: la matriz
# y ingresa por teclado

def vec_prueba(vec_paises, y):
    """
